...
const redirectURI = `https://apfbkfkcgajohfclgbedbbjjllfcldjb.chromiumapp.org/provider_cb`; // 서비스 URL로 확장프로그램의 ID : pbdbeakfknimebmfgmflffaabppbfbmn 를 이용하며 뒤에 .chromiumapp.org/provider_cb를 붙여야함
const supabaseFunctionURL =
  "https://ykorbmtrpjhatgnhbjbp.supabase.co/functions/v1/get-naver-token"; // 로그인 시도 supabase Function 요청 URL
...
async function fetchUserInfoFromSupabase(accessToken) {
  const response = await fetch(
    `${supabaseUserInfoURL}?accessToken=${accessToken}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization:
          "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inlrb3JibXRycGpoYXRnbmhiamJwIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTcyNTg0ODU1MywiZXhwIjoyMDQxNDI0NTUzfQ.vzkgJHBJr4E2vTjAYjq08nbPK1Y9w0J4dLckOxJ9Udk",
      },
    }
  );
